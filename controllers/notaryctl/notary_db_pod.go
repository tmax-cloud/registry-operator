...
func (nt *NotaryDBPod) Ready(c client.Client, notary *regv1.Notary, patchNotary *regv1.Notary, useGet bool) error {
	...
	condition := &status.Condition{
		...
		Type:   regv1.ConditionTypeNotaryDBPod,
	}
...
func (nt *NotaryDBPod) create(c client.Client, notary *regv1.Notary, patchNotary *regv1.Notary, scheme *runtime.Scheme) error {
	var err error = nil
	condition := &status.Condition{
		Status: corev1.ConditionFalse,
		Type:   regv1.ConditionTypeNotaryDBPod,
	...
	nt.logger.Info("Create notary db pod")
	if err = c.Create(context.TODO(), nt.pod); err != nil {
		nt.logger.Error(err, "Creating notary db pod is failed.")
		return nil
	}
...
func (nt *NotaryDBPod) get(c client.Client, notary *regv1.Notary) error {
	nt.pod = schemes.NotaryDBPod(notary)
	...
	if err := c.Get(context.TODO(), req, nt.pod); err != nil {
		nt.logger.Error(err, "Get notary db pod is failed")
		return err
...
func (nt *NotaryDBPod) delete(c client.Client, patchNotary *regv1.Notary) error {
	var err error = nil
	condition := &status.Condition{
		Status: corev1.ConditionFalse,
		Type:   regv1.ConditionTypeNotaryDBPod,
